...
This project presents an automated ETL (Extract, Transform, Load) pipeline for foreign exchange rate data, built with Python and orchestrated using Prefect. The system extracts real-time exchange rate data from exchangerate.host, performs schema validation through Pydantic, applies transformations such as rate rounding and timestamp formatting, and stores the processed data in both CSV and SQLite formats. Furthermore, a web interface was developed to visualize the exchange rates and support real-time currency conversion. This work demonstrates how modern data validation frameworks and orchestration tools can be effectively combined to automate and monitor critical data workflows with reliability and clarity.
